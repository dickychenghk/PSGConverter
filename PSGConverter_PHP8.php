...
class Midi{
    ...
    // 声明属性以防止 PHP 8+ 动态属性弃用
    public array $tracks;
    public int $timebase;
    public ?int $tempo = null;          // tempo as integer (0 for unknown)
    public ?int $tempoMsgNum = null;     // position of tempo event in track 0
    ...
    ****************************************************************************/
    ...
    public function getMid(): string {
        ...
        $tc = count($tracks);
        $typeVal = ($tc > 1) ? 1 : 0;
        $midStr = "MThd\0\0\0\6\0" . chr($typeVal) . $this->_getBytes($tc, 2) . $this->_getBytes($this->timebase, 2);
        for ($i = 0; $i < $tc; $i++) {
            $track = $tracks[$i];
            $mc = count($track);
            $time = 0;
    ...
    //---------------------------------------------------------------
    public function downloadMidFile(string $output, string $file = ''): void {
    ...
    private function _getMsgStr(string $line): string {
        ...
        switch ($msg[1]) {
            ...
            case 'Seqnr': // 0x00 = sequence_number
                $num = chr((int)$msg[2]);
                if ((int)$msg[2] > 255) _err("code broken around Seqnr event");
...
// MM: Todo: Len could also be more than one Byte (variable length; see. "Sequence/Track name specification)
                        $len = chr(strlen($txt));
                        if (ord($len) > 127) _err("code broken (write varLen-Meta)");
...
// MM: ToDo: Len >127 has to be variable length-encoded !!!
    ...
    private function _getBytes(int $n, int $len): string {
        $str = '';
        for ($i = $len - 1; $i >= 0; $i--) {
            $str .= chr((int) floor($n / pow(256, $i)));
        }
        return $str;
    }

    //---------------------------------------------------------------
    // int to variable length string (32-bit compatible)
    //---------------------------------------------------------------
    private function _writeVarLen(int $value): string {
        // Ensure 32-bit behavior even on 64-bit PHP
        $value &= 0xFFFFFFFF;
        $buf = $value & 0x7F;
        $str = '';
        while (($value >>= 7) > 0) {
    ...
    // error message
    //---------------------------------------------------------------
    private function _err(string $str): void {
...
// hexstr to binstr
//---------------------------------------------------------------
function _hex2bin(string $hex_str): string {
    $bin_str = '';
    for ($i = 0; $i < strlen($hex_str); $i += 2) {
        $bin_str .= chr(hexdec(substr($hex_str, $i, 2)));
    }
...
// int to bytes (length $len)
//---------------------------------------------------------------
function _getBytes(int $n, int $len): string {
    $str = '';
    for ($i = $len - 1; $i >= 0; $i--) {
        $str .= chr((int) floor($n / pow(256, $i)));
    }
...
//---------------------------------------------------------------
// int to variable length string (standalone, 32-bit compatible)
//---------------------------------------------------------------
function _writeVarLen(int $value): string {
    $value &= 0xFFFFFFFF;
    $buf = $value & 0x7F;
    $str = '';
    while (($value >>= 7) > 0) {
...
if (!isset($_REQUEST['m'])) {
    // 音色番号→$inst
    $inst   = (isset($_REQUEST['i'])) ? (int)$_REQUEST['i'] : 1;    // 楽器設定
    $isDrum = (isset($_REQUEST['d'])) ? (int)$_REQUEST['d'] : 0;    // ドラムパートか？
    $ch     = (isset($_REQUEST['c'])) ? (int)$_REQUEST['c'] : (($isDrum !== 0) ? 10 : 1);    // 使用チャンネル（ドラムだった場合は、10にする）
    if(($inst<1) || (128<$inst)) $inst=1;    // 楽器の範囲は、1~128
    // 2.1 Add
    $Hlimit = (isset($_REQUEST['h'])) ? (int)$_REQUEST['h'] : 88;    // 音階の最高値
    $Llimit = (isset($_REQUEST['l'])) ? (int)$_REQUEST['l'] : 16;    // 音階の最低値
} else {
    // プリセットの楽器番号を使う（0は無効。楽器設定などのパラメータは無視されます。3MLE互換）
    switch ((int)$_REQUEST['m']){
        case 1:    // リュート
            $inst = 24;
            $Hlimit = 88;
            $Llimit = 16;
...
$effect = (isset($_REQUEST['e'])) ? (int)$_REQUEST['e'] : 40;    // エフェクトの量
$pan    = (isset($_REQUEST['p'])) ? (int)$_REQUEST['p'] : 64;    // パンポッド

if(isset($_REQUEST['s'])) { 
    $mml = rawurldecode($_REQUEST['s']);
}else {
...
// MMLをパース。
preg_match(
    '/(MML@)\s*([\s0-9a-glnortvA-GLNORTV#<>.&+-]*),\s*([\s0-9a-glnortvA-GLNORTV#<>.&+-]*),\s*([\s0-9a-glnortvA-GLNORTV#<>.&+-]*);/',
    $mml,
    $matches
);
if ($matches && $matches[1] == "MML@"){
    if($matches[2] != ''){ 
        ...
        $midiText[] = '193 Par ch=' . $ch . ' c=10 v=' . $pan; // パンポッド
...
function PSGConverter(string $mml, int $ch, int $isDrum, string $max, string $min): string {
    $mml = preg_replace('/\r|\n/','',$mml);
    // 命令の取得
    // $mmlを命令単位に分解して格納する
    ...
    foreach($matches[0] as $mml_note) { // 1命令→メッセージ変換
            ...
            switch ($RegExp[1]){
                ...
                case 'l': case 'L':
                    if ((1 <= $value) && ($value <= $minim)) {
                        $cLength = (int) floor($semibreve / $value);    // L1 -> 384tick .. L64 -> 6tick
        ...
        if (preg_match('/([a-gnA-GN])([\+\#-]?)([0-9]*)(\.?)(&?)/',$mml_note,$RegExp)){
            ...
            if ($isDrum == 0){
                ...
                while ($note < (int)$min) $note = $note + 12;
                while ($note > (int)$max) $note = $note - 12;
